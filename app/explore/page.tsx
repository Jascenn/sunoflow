...
import { Compass, Play, Loader2, Music } from 'lucide-react';
...
export default function ExplorePage() {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[50vh]">
                ...
                <p className="text-gray-500">{t('explore.discovering')}</p>
            </div>
        );
    }
        ...
        return (
            <div className="p-8 text-center text-gray-500">
                <div className="mb-4">
                    ...
                    <h2 className="text-xl font-bold text-gray-700">{t('sidebar.explore')}</h2>
                </div>
                <p>{error || t('explore.no_music')}</p>
            </div>
        );
    }
    ...
    return (
        <div className="p-8">
            ...
            <section>
                <h2 className="text-lg font-semibold mb-4">{t('explore.trending_now')}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {data.map((track, i) => (
                        <div key={track.id} className="group bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden cursor-pointer flex flex-col h-full">
                            <div className="h-40 w-full bg-gray-100 relative flex items-center justify-center overflow-hidden shrink-0">
                                {track.imageUrl ? (
                                    <img src={track.imageUrl} alt={track.title} className="w-full h-full object-cover transition-transform group-hover:scale-105" />
                                ) : (
                            ...
                            <div className="p-4 flex flex-col flex-1">
                                <h3 className="font-bold text-gray-900 line-clamp-1 mb-1">{track.title || t('explore.untitled')}</h3>
                                ...
                                <div className="flex items-center justify-between mt-3 text-xs text-gray-400 border-t border-gray-50 pt-2">
                                    <span>{track.duration ? `${Math.floor(track.duration)}s` : ''}</span>
                                    <span className="bg-gray-50 px-1.5 rounded font-mono">{track.id.slice(0, 6)}</span>
                                </div>
                            </div>
                        </div>
                    ))}
